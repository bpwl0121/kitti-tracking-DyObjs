...
def get_moving_mask_dict(all_moving_id_dict, mots_mask_datasets, all_obj_info_dict, association_threshold):
    # https://github.com/xinshuoweng/AB3DMOT/blob/6698c95606d819c03c9215b42f6f652b635a324d/AB3DMOT_libs/matching.py
    moving_mask_dict = dict()
    ignored_mask_dict = dict()
    for seq_num in all_moving_id_dict.keys():
        moving_mask_dict[seq_num] = dict()
        ignored_mask_dict[seq_num] = dict()
        for frame_num in all_moving_id_dict[seq_num]:

            if frame_num not in mots_mask_datasets[seq_num]:
                continue

            decoded_mots_mask_list = list()
            decoded_ignored_mots_mask_list = list()
            for seg_obj in mots_mask_datasets[seq_num][frame_num]:
                if seg_obj.class_id == 10:
                    decoded_mots_mask = rletools.decode(seg_obj.mask)
                    decoded_ignored_mots_mask_list.append(decoded_mots_mask)
                # car and pedestrian
                else:
                    decoded_mots_mask = rletools.decode(seg_obj.mask)
                    decoded_mots_mask_list.append(decoded_mots_mask)
            ...
            if len(decoded_mots_mask_list) != 0 and len(moving_bbox_list) != 0:
                ...
                if len(matches_mask_ind) == 0:
                    ...
                    moving_mask_dict[seq_num][frame_num] = moving_mask

              
            if len(decoded_ignored_mots_mask_list):
                ignored_mask_dict[seq_num][frame_num] = sum(
                    decoded_ignored_mots_mask_list)

    return moving_mask_dict, ignored_mask_dict
