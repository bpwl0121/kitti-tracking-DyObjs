...
class MovingMaskAssociator:
    def __init__(self, association_threshold):
    ...
    def compute_overlaps(boxes1, boxes2):
        ...
        for i in range(overlaps.shape[1]):
            box2 = boxes2[i]
            overlaps[:, i] = MovingMaskAssociator.compute_iou(
                box2, boxes1, area2[i], area1)
        return overlaps
    ...
    def get_moving_mask_dict_detectron(all_moving_id_dict, detectron_img_folder, all_obj_info_dict, association_threshold):
        ...
        os.makedirs(os.path.join(
            "moving_mask_detectron_index",), exist_ok=True)
        ...
        for seq_num in all_moving_id_dict.keys():
            ...
            for frame_num in all_moving_id_dict[seq_num]:
                ...
                if len(decoded_mots_mask_list) != 0 and len(moving_bbox_list) != 0:
                    ...
                    decoded_mots_masks = np.stack(
                        decoded_mots_mask_list, axis=2)
                    # extract bbox from segmentation mask
                    seg_obj_2d_bboxs = MovingMaskAssociator.extract_bboxes(
                        decoded_mots_masks)

                    all_moving_bboxs = np.stack(moving_bbox_list, axis=0)

                    # compute overlaps between moving bbox and segmentation mask
                    aff_matrix = MovingMaskAssociator.compute_overlaps(
